...
class GeneratedScene(Scene):
    def construct(self):
        ...
        self.play(
            Transform(lbl_step, Text("Step 2: sin x を二乗 → sin^2 x", font_size=26).to_edge(UP).shift(DOWN * 0.9))
        )
        ...
        self.play(
            Transform(lbl_step, Text("Step 4: cos x を二乗 → cos^2 x", font_size=26).to_edge(UP).shift(DOWN * 0.9))
        )
        ...
        self.play(
            Transform(
                lbl_step, Text("Step 5: sin^2 x + cos^2 x を表示（常に 1）", font_size=26).to_edge(UP).shift(DOWN * 0.9)
            )
        )
