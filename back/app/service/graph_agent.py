...
class ManimGraphAnimationService(BaseManimAgent):
    ...
    def generate_script_with_prompt(self, contents, video_enhance_prompt):
        ...
        manim_script_prompt = PromptTemplate(
            input_variables=["instructions"], template=self.prompts["chain"]["manim_script_generate"]
        )
        # --- 2. チェーンを分割して定義 ---
        ...
        chain = RunnablePassthrough.assign(planner_output=planner_chain).assign(
            script_output=(lambda x: {"instructions": x["planner_output"]}) | script_chain
        )
        # --- 4. チェーンの実行と結果の取得 ---

        output_dict = chain.invoke({"user_prompt": contents, "video_enhance_prompt": video_enhance_prompt})
    ...
    def _generate_initial_script(self, state: ManimGraphState):
        ...
        planner_script, script = self.generate_script_with_prompt(
            state["user_request"], state["generation_instructions"]
        )
        self.base_logger.debug(f"   [+] Initial script generated (length: {len(script)})")
        return {"current_script": script, "current_retry": 0, "animation_plan": planner_script}
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            "check_bad_request", self._after_bad_request_check, {"end_with_error": END, "execute": "execute"}
        )
        workflow.add_conditional_edges(
            "execute", self._after_execution, {"refine": "refine", "end_with_success": END, "end_with_error": END}
        )
        return workflow
